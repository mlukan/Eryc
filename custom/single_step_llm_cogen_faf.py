...
class SingleStepLLMCoGenFaF(SingleStepLLMCommandGenerator):
    ...
    async def _predict_commands(
        ...
        self._update_message_parse_data_for_fine_tuning(message,action_list, commands, flow_prompt)
        structlogger.debug(f"LLM cogen FAF commands: {commands}")
        slot_feedback = tracker.get_slot("slot_feedback")
        slot_squat = tracker.get_slot("slot_squat")
        if slot_feedback and slot_feedback == "pending":
            commands.append(SetSlotCommand(name="slot_feedback", value="pass"))
        if slot_squat:
            commands = [command for command in commands if not isinstance(command, StartFlowCommand)]
        structlogger.debug(f"LLM cogen FAF filtered commands: {commands}")
        return commands

    @staticmethod
    def _update_message_parse_data_for_fine_tuning(
        message: Message, raw_llm_response: str, commands: List[Command], prompt: str
    ) -> None:
        ...
        if preparing_fine_tuning_data:
            ...
            message.set("raw_llm_output", raw_llm_response, add_to_output=True)
